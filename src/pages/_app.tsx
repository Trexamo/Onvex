...
const noCommunityButtonRoutes = ['/login', '/register', '/chat'];

export default function App({ Component, pageProps }: AppProps) {
  const router = useRouter();
  const showSidebar = !noSidebarRoutes.includes(router.pathname);
  const showCommunityButton = !noCommunityButtonRoutes.includes(router.pathname);
...
'@ | Out-File -FilePath _app.tsx -Encoding UTF8